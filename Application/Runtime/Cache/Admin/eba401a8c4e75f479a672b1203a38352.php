...
<script type="text/javascript" src="/Public/admin/js/engine.js"></script>
	<section>
		...
		<script type="text/javascript">
            ...
            var pckzs = {
                activate: function (user_name) {
                    ...
                    var ajax = m.ajax("<?php echo U('Admin/Daili/recmlist_activate');?>", null, function (jso) {
		...
		<div class="content-wrapper">
			...
			<h3>
				代理中心 <?php echo ($centername); ?> 会员列表
				<input type="hidden" id="cenname" value="<?php echo ($centername); ?>">
			</h3>
			<a href="#" onclick="history.back(-1)"><button class="btn btn-success btn-sm savecode">返回</button></a>
			<!-- 
			<div style="text-align:right;">
				<a class="btn btn-primary mb-lg" href="<?php echo U('Daili/center_detail',array('user_name'=>$centername));?>" target="main-frame">查看申请历史</a>
			</div>
			 -->
...
<script>
    ...
    function getpage(p){
        ...
        $("#list").load(
            "<?php echo U('center_info_ajax');?>?centerid="+centerid+"&user_name="+user_name+"&p="+p+"&truename="+truename+"&level="+level+"&rusername="+rusername+"&pusername="+pusername+"&type="+type+"&start="+start,
            function() {}
        );
    }
