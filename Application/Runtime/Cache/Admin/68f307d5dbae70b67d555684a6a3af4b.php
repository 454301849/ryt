...
<section>
	<div class="content-wrapper">
		...
		<ul class="nav nav-tabs" role="tablist" >
			<li  role="presentation" class="active"><a style="background-color:transparent;border-radius:3px;">编辑角色</a></li>
		</ul>
					...
					<table class="table table-striped table-bordered table-hover" style="margin-top: 25px;">
						...
						<tr >
							...
							<td><div class="controls">
								<input type="text"  class="form-control"  id="rolename" name="name" maxlength="20" placeholder="" value="<?php echo ($data["name"]); ?>" style="width: 30%;">
							</div></td>
						</tr>
						<tr >
							<td class="td-title col-sm-2 ">角色描述：</td>
							<td><div class="controls">
								<textarea name="remark" rows="2" cols="20" id="remark" class="form-control" maxlength="100"  style="height: 100px; width: 500px;"><?php echo ($data["remark"]); ?></textarea>
							</div></td>
						</tr>
						<tr>
							...
							<td>
								<div class="controls">
									...
									<label class="radio inline addradio" for="active_true">
										<input type="radio" name="status" <?php echo ($active_true_checked); ?> value="1" checked id="active_true" />开启</label>
									<label class="radio inline addradio" for="active_false">
										<input type="radio" name="status" value="0" id="active_false" <?php echo ($active_false_checked); ?>>禁用</label>
								</div>
							</td>
						</tr>
	...
	<script type="text/javascript">
		var pck = {
			submit: function () {
				...
				var ajax = m.ajax("<?php echo U('Admin/Rbac/roleedit_post');?>", null, function (jso) {
					var jso = m.json.getObject(jso);
					switch (jso.Error) {
						case 0:
							engine.news("修改角色成功", 3000);
							setTimeout(function () {
