...
class ShopController extends ActionController
{
	...
	function good()
	{
		...
		$good_list = $shop_goods->order("good_id desc")->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$show = $Page->pc_show();
...
// 			dump($_POST);
	...
	function order()
	{
		...
		foreach ($order_info as $k => $v) {
			...
			$order_info[$k]['address'] = $this->getaddress($v['address_id']);
		}
	...
	function getaddress($address_id){
        ...
        $info = $user_address->where("address_id = '{$address_id}'")->find();
        return $info;
    }
	...
	function order_more()
	{
		...
		$user_address = M('address')->getByUser_id($order['user_id']);
