...
class UserController extends ActionController
{
    ...
    public function register(){
        ...
        //测试
        	$list = M('broke_record')->field('id,user_name,yuser_name')->select();
        	foreach ($list as $v){
        		$b = implode(explode('M', $v['user_name']));
        		$a = implode(explode('M', $v['yuser_name']));
        		$options['user_name'] = 'M00'.$b;
        		$options['yuser_name'] = 'M00'.$a;
				M('broke_record')->where("id = ".$v['id'])->save($options);
        	}
		//测试
        die();
