...
class CenterController extends Controller{
	...
	public function index(){
		...
		$level_info = $this->getlevel($user_info['level']);
		$this->assign("userinfo",$user_info);
		$this->assign("levelinfo",$level_info);
		$this->display('index1');
	}
	...
	*/
	...
	{
		return M('memberlevel')->where(" id = '{$level}' ")->find();
     ...
     * 验证登录密码
     */
	public function api_login_submit(){
        ...
        $info =$users->where("moblie= '{$moblie}' or user_name = '{$moblie}' ")->select();
        if ($info[0]['password'] == md5($password)) {
            session(null);
            session('user_id', $info[0]['user_id']);
            session('user_name', $info[0]['user_name']);
     ...
     * 验证二级密码
     */
    public function api_authentication_submit(){
        $users = M('users');
        $user_id = $_SESSION['user_name'];
        $password = $_POST['safeword'];
        $info =$users->where("user_name= '{$user_id}' ")->select();
        if ($info[0]['second_password'] == md5($password)) {
            session('two_Verify', true);
            $msg['Error'] = 0;
            echo json_encode($msg);
            exit;
        } else {
            $msg['Error'] = '-1';
            $msg['sql'] = $_SESSION['user_id'].$users->getLastSql();
            $msg['Data'] = "映射失败";
            echo json_encode($msg);
            exit;
        }
    ...
    function centerlists(){
    	...
    	$show = $Page->wap_show();
    	$user_info = $users->where("user_name ='$centerid' ")->find();
    	$this->assign('info',$info);
